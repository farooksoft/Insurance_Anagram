...
game.displayLossScreen = function() {
    ...
    // Determine the SUVI message based on the final score
    var suviMessage;
    var finalScore = parseInt(game.scoreNum);

    if (finalScore < 7) {
        suviMessage = '0 SUVI';
    } else if (finalScore >= 7 && finalScore < 15) {
        suviMessage = '100 SUVI';
    } else if (finalScore >= 15) {
        suviMessage = '200 SUVI';
    }

    // Add the SUVI message to the loss screen (or any other element where you want to display it)
    var suviElement = document.getElementById('suvi_message');
    if (!suviElement) {
        suviElement = document.createElement('div');
        suviElement.id = 'suvi_message';
        game.lossScreen.appendChild(suviElement);
    }
    suviElement.innerHTML = suviMessage;
